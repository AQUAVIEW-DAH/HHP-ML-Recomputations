"""Compare base vs global-physics feature sets for 2024-train / 2025-test XGBoost."""
...
FEATURE_SETS = {
    "base": BASE_FEATURES,
    "base_plus_global_physics": BASE_FEATURES + PHYSICS_FEATURES,
    "base_plus_global_physics_pruned": BASE_FEATURES + [
        "model_ssh_m",
        "model_mixed_layer_thickness_m",
        "model_surface_boundary_layer_thickness_m",
        "model_temp_excess_26c",
...
def _metric_row(*, target: str, model: str, split: str, y_true, y_pred, rows: int, dates: int) -> dict:
    return {
        ...
        "rmse": float(_rmse(y_true - y_pred, np.zeros_like(y_true))),
...
def run_target(df: pd.DataFrame, target, feature_set_name: str, feature_cols: list[str]) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    rows: list[dict] = []
    rows.append(_metric_row(target=target.name, model="raw_rtofs", split="2025_all", y_true=y_test_obs, y_pred=raw_test, rows=len(pred_df), dates=pred_df["date"].nunique()))
    rows.append(_metric_row(target=target.name, model=feature_set_name, split="2025_all", y_true=y_test_obs, y_pred=pred_test, rows=len(pred_df), dates=pred_df["date"].nunique()))
    for group_name, col in [("season", "season_group"), ("region", "region_group")]:
        for key, gdf in pred_df.groupby(col):
            ...
            rows.append(_metric_row(target=target.name, model=feature_set_name, split=f"2025_{group_name}:{key}", y_true=y_true, y_pred=gdf[f"pred_obs__{feature_set_name}"].to_numpy(float), rows=len(gdf), dates=gdf["date"].nunique()))
...
def main() -> None:
    ...
    for feature_set_name, feature_cols in FEATURE_SETS.items():
        for target in TARGETS:
            ...
            metrics_df.to_csv(OUT_DIR / f"year_holdout_global_physics_metrics_{target.name}_{feature_set_name}.csv", index=False)
            pred_df.to_parquet(OUT_DIR / f"year_holdout_global_physics_predictions_{target.name}_{feature_set_name}.parquet", index=False)
            fi_df.to_csv(OUT_DIR / f"year_holdout_global_physics_importance_{target.name}_{feature_set_name}.csv", index=False)
    ...
    metrics.to_csv(OUT_DIR / "year_holdout_global_physics_metrics_all.csv", index=False)
    importance.to_csv(OUT_DIR / "year_holdout_global_physics_importance_all.csv", index=False)
    summary.to_csv(OUT_DIR / "year_holdout_global_physics_summary.csv", index=False)
    ...
    payload = {
        ...
        "summary_csv": str(OUT_DIR / "year_holdout_global_physics_summary.csv"),
        "importance_csv": str(OUT_DIR / "year_holdout_global_physics_importance_all.csv"),
    }
    (OUT_DIR / "year_holdout_global_physics_run_summary.json").write_text(json.dumps(payload, indent=2))
