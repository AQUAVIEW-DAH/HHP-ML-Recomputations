"""Render side-by-side winter/summer RTOFS seasonal mean maps."""
...
def load_field(year: int, season_key: str, param_key: str) -> tuple[np.ndarray, np.ndarray, np.ndarray, int]:
    path = INPUT_DIR / f'rtofs_{year}_{season_key}_mean.nc'
    with xr.open_dataset(path) as ds:
        lat = ds['Latitude'].values
        lon = ds['Longitude'].values
        field = ds[param_key].values
        n = int(ds.attrs.get('n_daily_fields', 0))
    return lat, lon, field, n


def render_year_param(year: int, param_key: str, out_dir: Path, *, smooth_display: bool = False) -> Path:
    ...
    for ax, (season_key, (season_label, _months)) in zip(axes, SEASONS.items()):
        lat, lon, field, n = load_field(year, season_key, param_key)
    ...
    fig.suptitle(f'{cfg.label} RTOFS seasonal mean maps ({year})\nstyle={style_label}', fontsize=14)
    cbar = fig.colorbar(mappable, ax=axes.ravel().tolist(), shrink=0.85, pad=0.02)
    cbar.set_label(cfg.label)

    out_path = out_dir / f'{param_key}_{year}_rtofs_native_{style_label}.png'
...
def main() -> None:
    ...
    parser.add_argument('--output-subdir', default='rtofs_native')
    ...
    for param_key in args.params:
        for year in args.years:
            out = render_year_param(year, param_key, out_dir, smooth_display=args.smooth_display)
            print(out)
